...
for process in range(numProcesses):
    ...
    for guardNum in range(numGuards):
        ...
        #src port
        # add restrictions on destination port
        # destination port cannot be outside the range given below
        guard = "guard" + str(guardNum + 1) +" 123\n"
        guard += "< " + lowerBound + " tcp.src_port\n"
        guard += "> " + upperBound + " tcp.src_port\n"
        guard += "< " + lowerBound + " tcp.dst_port\n"
        guard += "> " + upperBound + " tcp.dst_port\n"
        guard += "AND ^0 ^1\n"
        guard += "AND ^2 ^3\n"
        guard += "AND ^4 ^5"
