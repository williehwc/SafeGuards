...
def send_and_receive_message(operation_type, content, request_id, pid, rsa_key):#, public_key_safeguards):
...
class SafeGuardsThread (Thread):
    ...
    def run(self):
        ...
        for process in range(start, end):
            processTime = 0
            send_and_receive_message('k', public_key, request_id, process + 1, rsa_key)#, public_key_safeguards)
            request_id += 1
            for guardNum in range(numGuards):
                ...
                guard += "AND ^0 ^1"
                guardTime = send_and_receive_message('i', guard, request_id, process + 1, rsa_key)#, public_key_safeguards)
                request_id += 1
                print(guardTime)
...
def main():
    ...
    # process 1 and 2
    firstThread = SafeGuardsThread(0, 1, 1, 0)
    # process 3 and 4
    #secondThread = SafeGuardsThread(1, 2, 1, 20)
